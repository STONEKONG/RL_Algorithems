...
class main():
    ...
    def train(self):
        ...
        onehot_actions = tf.one_hot(
            actions_h, self.env.actions, dtype=tf.float32)

        online_Q = tf.reduce_sum(tf.multiply(
            online_net['Q_value'], onehot_actions), axis=1)
        ...
        online_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope='online_net')
        ...
        with tf.Session(config=config) as sess:
            ...
            for episode in range(args.episodes):
                ...
                for step in range(args.max_episode_len):
                    ...
                    episode_buffer.add(
                        np.reshape(
                            np.array([state, action, reward, state_1, d]), [1, 5])
                    )
                    total_step += 1
                    if total_step > args.pre_train_step:
                        ...
                        Q_value_1 = sess.run(
                            target_net['Q_value'],
                            feed_dict={
                                target_net['state']: np.stack(
                                    batch_data[:, 3], axis=0)
                            }
                        )
                        end_multiplier = -(batch_data[:, 4] - 1)
                        max_Q_1 = np.max(Q_value_1, 1)
                        target_Q = batch_data[:, 2] + \
                            args.gamma * max_Q_1 * end_multiplier
                        _ = sess.run(
                            update_model,
                            feed_dict={
...
def Args():
    ...
    parser.add_argument('--h_size', type=int, default=512,
                        help='size of hiden space')
    parser.add_argument('--buffer_size', type=int,
                        default=50000, help='experence buffer size')
    ...
    parser.add_argument('--max_epsilon', type=float, default=1.0,
                        help='max probility of greedy algorithm')
    parser.add_argument('--min_epsilon', type=float,
                        default=0.15, help='max probility of greedy algorithm')
