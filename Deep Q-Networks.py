...
class main():
    ...
    def cnn(self, input_img, name):
        ...
        with tf.variable_scope(name):
            ...
            advantage = slim.fully_connected(stream_a, env.actions, activation_fn=None)
    ...
    def train(self):
        ...
        onehot_actions = tf.one_hot(actions_h, env.actions, dtype=tf.float32)
        ...
        with tf.Session(config=config) as sess:
            ...
            for episode in range(args.episodes):
                ...
                if len(r_list) % 10 == 0:
                    print('****', total_step, np.mean(r_list[-10:]), epsilon)

                if episode % 500 == 0:
                    model_path = os.path.join(args.save_path, 'agent.ckpt-%d-%.2f' % (episode, float(np.mean(r_list[-10:]))))
...
def Args():
    ...
    parser.add_argument('--min_epsilon', type=float, default=0.1, help='')
